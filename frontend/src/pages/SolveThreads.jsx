  ...
  <>
    ...
    {!loading && hasMore && (
      <div className="mt-3 flex justify-center">
        ...
        {/* Property Details Drawer (similar to RoommateMatches) */}
        {detailFor && (
          <div className="fixed inset-0 z-50 transition" aria-hidden={false}>
            ...
            />
            <aside
              className="absolute right-0 top-0 h-full w-full max-w-[700px] overflow-y-auto border-l border-white/10 bg-[#0d1017] shadow-2xl transition-transform will-change-transform translate-x-0"
              role="dialog"
              aria-modal="true"
              ...
              <div className="px-5 pb-10 pt-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                      <div className="relative h-14 w-14 overflow-hidden rounded-full bg-white/10">
                        {(() => {
                          const img = primaryImageUrl(detailFull || detailFor);
                          return img ? (
                            <img src={img} alt={detailFull?.title || detailFor?.title} className="h-full w-full object-cover" />
                          ) : (
                            <div className="flex h-full w-full items-center justify-center text-sm text-white/70">
                              {detailFull?.title || detailFor?.title || "Property"}
                            </div>
                          );
                        })()}
                      </div>
                    ...
                    {typeof (detailFull?.matchScore ?? detailFor?.matchScore) === "number" && (
                      <div className="rounded-2xl bg-black/40 p-2 border border-white/10 shadow-[0_0_30px_rgba(56,189,248,0.15)]">
                        <div className="flex items-center justify-center w-14 h-14 rounded-full bg-gradient-to-r from-emerald-500 to-cyan-500 text-white font-bold text-sm">
                          {Math.round(detailFull?.matchScore ?? detailFor?.matchScore)}%
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="mt-6">
                  <div className="w-full overflow-hidden rounded-lg bg-white/5">
                    <div className="h-[300px] max-h-[50vh]">
                      {(() => {
                        const imgs = collectAllImages(detailFull, detailFor);
                  ...
                  {(() => {
                    ...
                    return (
                      <div className="mt-2 flex items-center justify-between">
                        <button onClick={prev} className="rounded-md bg-white/5 px-3 py-1 text-sm text-white/80 hover:bg-white/10">
                          Prev
                        </button>
                        <div className="text-xs text-white/60">
                          {detailPhotoIndex + 1} / {imgs.length}
                        </div>
                        <button onClick={next} className="rounded-md bg-white/5 px-3 py-1 text-sm text-white/80 hover:bg-white/10">
                          Next
                        </button>
                      </div>
                    );
                ...
                {/* Key facts */}
                <div className="mt-6 grid grid-cols-2 gap-3 text-[14px] sm:grid-cols-3">
                  {(() => {
                    ...
                    return facts.map((f, i) => (
                      <div key={i} className="flex items-center gap-2 rounded-lg border border-white/10 bg-white/[0.03] px-2.5 py-2">
                        <div className="text-[18px] text-white/70">{f.icon}</div>
                        <div className="min-w-0">
                          <div className="text-[12px] uppercase tracking-wide text-white/50">{f.label}</div>
                          <div className="truncate text-white/80">{f.value}</div>
                        </div>
                      </div>
                    ));
                  })()}
                ...
                {(() => {
                  ...
                  return (
                    <div className="mt-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/0 p-4">
                      <h4 className="text-sm font-semibold tracking-wide text-white/80 mb-2">About this place</h4>
                      <p className="whitespace-pre-wrap text-[14px] text-white/80">{desc}</p>
                    </div>
                  );
                })()}
                ...
                {(() => {
                  ...
                  return (
                    <div className="mt-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/0 p-4">
                      <h4 className="text-sm font-semibold tracking-wide text-white/80 mb-2">Address</h4>
                      <div className="text-white/80">{addr}</div>
                    </div>
                  );
                })()}
                ...
                {/* Notes */}
                <div className="mt-6 rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/0 p-4">
                  <h4 className="text-sm font-semibold tracking-wide text-white/80 mb-2">Your notes</h4>
                  <textarea
                    rows={3}
                    value={notes[detailFor._id] || ""}
                    onChange={(e) => saveNote(detailFor._id, e.target.value)}
                    className="w-full rounded-md border border-white/10 bg-white/5 p-2 text-[14px] outline-none text-white placeholder-white/40"
                    placeholder="e.g., Ask about utilities, FaceTime tour at 5pm, roommate likes cats…"
                  />
                </div>

                {/* CTA */}
                <div className="mt-8 sticky bottom-0 left-0 right-0 z-10 border-t border-white/10 bg-[#0d1017]/90 backdrop-blur supports-[backdrop-filter]:bg-[#0d1017]/60">
                    ...
                    {(() => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isDisabled(state)}
                          className={cx(
                            "flex-1 rounded-xl border border-white/10 px-4 py-3 text-sm font-medium",
                            state === "approved"
                              ? "bg-emerald-600 text-white"
                              : state === "pending"
                              ? "bg-amber-500 text-black"
                  ...
                  {detailLoading && (
                    <div className="px-5 pb-2 text-xs text-white/60">Loading more details…</div>
                  )}
                </div>
              </div>
            </aside>
