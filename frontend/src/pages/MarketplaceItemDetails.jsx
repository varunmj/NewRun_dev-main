// src/components/Cards/MarketplaceItemCard.jsx
import React from "react";
import { MdFavoriteBorder, MdFavorite, MdArrowForwardIos } from "react-icons/md";
...
export default function MarketplaceItemCard({ item, favored = false, onToggleFav, onClick }) {
      ...
      <div className="relative">
        ...
        <div className="absolute right-3 top-3">
          <button
            ...
            className="grid h-9 w-9 place-items-center rounded-full bg-black/45 text-white/95 ring-1 ring-white/10 backdrop-blur hover:bg-black/60"
          >
      ...
      <div className="px-5 pb-4 pt-4">
        ...
        <div className="mt-2 flex items-center justify-between gap-2">
          <div className="flex min-w-0 flex-wrap items-center gap-1.5 text-[12px] text-white/60">
            {deliveryLabel(item?.delivery) && <span className="rounded-full bg-white/5 px-2 py-0.5">{deliveryLabel(item.delivery)}</span>}
            {item?.condition && <span className="rounded-full bg-white/5 px-2 py-0.5 capitalize">{String(item.condition).toLowerCase()}</span>}
          </div>

          <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/8 text-white/85 transition-colors group-hover:bg-white/12">
