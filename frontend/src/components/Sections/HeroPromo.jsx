...
export default function HeroPromo() {
  ...
  const rotatingTaglines = [
    ...
    { text: "Your campus marketplace.", color: "#8b5cf6", glow: "0 0 6px #8b5cf6, 0 0 12px #8b5cf6" },
    { text: "Buy and sell with confidence.", color: "#f97316", glow: "0 0 6px #f97316, 0 0 12px #f97316" },
  ...
  // Note: Tagline rotation is now handled in the typing animation useEffect above
  ...
  // Count-up animation for active items
  useEffect(() => {
    if (isVisible) {
  ...
  const animateCountUp = () => {
    ...
    const updateCount = () => {
      ...
      setActiveItemsCount(currentCount);
  ...
  const handleSearchSubmit = (e) => {
    ...
    if (searchQuery.trim()) {
      ...
      const marketplaceGrid = document.getElementById('market-grid');
      if (marketplaceGrid) {
        marketplaceGrid.scrollIntoView({ 
          behavior: 'smooth',
          block: 'start'
        });
        // You could also dispatch a custom event or use a state management solution
        // to pass the search query to the marketplace component
        window.dispatchEvent(new CustomEvent('marketplaceSearch', { 
              ...
              <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-[#101215] px-4 py-3 hover:border-white/20 focus-within:border-amber-400/50 transition-all duration-200">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    placeholder={isSearchFocused ? "Search for items..." : `Try: ${searchSuggestions[searchIndex]}`}
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-2 backdrop-blur-sm">
            ...
            <span className="text-sm text-white/80">
              <span className="font-bold text-amber-400">
                {isCountUpVisible ? `${activeItemsCount}+` : '0+'}
              </span>
              <span className="ml-1">Active Items</span>
            </span>
          </div>
        </div>
